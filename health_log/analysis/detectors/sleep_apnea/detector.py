...
def assess_sleep_apnea_risk(
    ...
    *,
    ...
    recommendation = (
        "Для диагностики СОАС необходима полисомнография или домашний тест сна с пульсоксиметрией — "
        "Apple Watch не измеряет воздушный поток и не может поставить диагноз. "
        "Если паттерн повторяется или есть симптомы (дневная сонливость, громкий храп, остановки дыхания), "
        "обратись к сомнологу или пульмонологу."
    )

    interpretation = (
        "ВАЖНО: данные основаны исключительно на косвенных показателях (частота дыхания, ЧСС, ВСР) — "
        "Apple Watch не измеряет воздушный поток. Специфичность скрининга низкая: возможны "
        "ложноположительные результаты. Индекс апноэ-гипопноэ (AHI) по этим данным не вычисляется. "
        "Это не диагноз и не замена полисомнографии."
    )
